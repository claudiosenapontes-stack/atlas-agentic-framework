...
const auditTypes: AuditType[] = [
    { id: 'fleet', label: 'Fleet Audit', icon: Users, description: 'Verify all agents online and responsive', wired: true },
    { id: 'systems', label: 'Systems Audit', icon: Server, description: 'Check PM2 services and health metrics', wired: false },
    { id: 'connections', label: 'Connections Audit', icon: Plug, description: 'Test integration connectivity', wired: false },
    { id: 'services', label: 'Services Audit', icon: Zap, description: 'Validate critical API endpoints', wired: false },
    { id: 'database', label: 'Database Audit', icon: Database, description: 'Check DB connectivity and performance', wired: false },
  ];
      ...
      <main className="p-4">
        ...
        <section className="mb-6">
          ...
          <div className="mb-3 p-2 bg-[#3B82F6]/5 rounded border border-[#3B82F6]/20 flex items-center gap-2">
            ...
            <span className="text-[10px] text-[#3B82F6]">Only Fleet Audit is wired. Others show backend truth state.</span>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
            {auditTypes.map((audit) => {
              ...
              // Wired audits: interactive buttons
              if (audit.wired) {
                return (
                  <button
                    key={audit.id}
                  ...
                  >
                    <div className="flex items-start justify-between mb-2">
                      ...
                      <span className="text-[9px] px-1.5 py-0.5 bg-[#16C784]/10 text-[#16C784] rounded">WIRED</span>
              ...
              // Unwired audits: disabled/placeholder
              return (
                <div
                  key={audit.id}
                  className="p-4 bg-[#111214] border border-[#1F2226] rounded-lg opacity-60 cursor-not-allowed"
                >
                  <div className="flex items-start justify-between mb-2">
                    <div className="w-10 h-10 rounded-lg bg-[#0B0B0C] flex items-center justify-center">
                      <Icon className="w-5 h-5 text-[#6B7280]" />
                    </div>
                    <span className="text-[9px] px-1.5 py-0.5 bg-[#6B7280]/10 text-[#6B7280] rounded">NOT WIRED</span>
                  </div>
                  <h3 className="text-sm font-medium text-[#9BA3AF] mb-1">{audit.label}</h3>
                  <p className="text-[10px] text-[#6B7280]">{audit.description}</p>
                  <p className="text-[10px] text-[#6B7280] mt-2">Backend endpoint not implemented</p>
                </div>
              );
            })}
          </div>
