...
import { PieChart, AlertCircle, RefreshCw, Radio } from 'lucide-react';
        ...
        {dataSource === 'unavailable' ? (
          <div className="flex flex-col items-center justify-center py-16 bg-[#111214] rounded-[10px] border border-[#1F2226]">
            <div className="w-16 h-16 rounded-full bg-[#1F2226] flex items-center justify-center mb-4">
              <PieChart className="w-8 h-8 text-[#6B7280]" />
            </div>
            ...
            <p className="text-sm text-[#9BA3AF] max-w-md text-center mb-6">
              Budget tracking is not yet available. Department budgets and spending will appear once the service is deployed.
            </p>
            <button 
              onClick={fetchBudgets} 
              disabled={loading} 
...
function BudgetCard({ budget }: { budget: Budget }) {
  ...
  const statusColors = { 
    ...
    frozen: 'bg-[#6B7280]/10 text-[#6B7280] border-[#6B7280]/30', 
    exceeded: 'bg-[#FF3B30]/10 text-[#FF3B30] border-[#FF3B30]/30' 
  };
  ...
  return (
    <div className="p-4 bg-[#111214] border border-[#1F2226] rounded-[10px]">
      <div className="flex items-start justify-between mb-3">
        ...
        <span className={`px-2 py-0.5 rounded text-[10px] border ${statusColors[budget.status]}`}>
          {budget.status}
        </span>
      </div>
      <div className="flex items-center justify-between text-sm mb-2">
        <span className="text-[#6B7280]">
          ${budget.spent.toLocaleString()} <span className="text-[#6B7280]">/ ${budget.allocated.toLocaleString()}</span>
        </span>
