...
import { Flame, Filter, SortAsc, RefreshCw, Plus } from 'lucide-react';
...
export default function HotLeadsPage() {
  ...
  const fetchLeads = async () => {
    ...
    try {
      // Use hot_only=true as per API spec
      ...
      // Map API response to HotLead interface
      const mappedLeads: HotLead[] = (data.leads || []).map((lead: any) => ({
        id: lead.id,
        name: lead.name,
        email: lead.email || '',
        ...
        sla_minutes: lead.sla_minutes || 30,
        due_at: lead.due_at || new Date(Date.now() + 30 * 60 * 1000).toISOString(),
      }));
      ...
      console.error('[HotLeads] Fetch error:', err);
  ...
  const handleClaimLead = async (leadId: string, agentId: string = 'claudio') => {
    try {
      ...
      console.error('[HotLeads] Claim error:', err);
      throw err;
    }
  };
  ...
  const stats: LeadStats = {
    ...
    overdue: leads.filter((l) => l.due_at && new Date(l.due_at) < new Date()).length,
  };
    ...
    <div className="min-h-screen bg-[#0B0B0C] text-white p-4 sm:p-6">
      ...
      <div className="flex flex-wrap items-center gap-3 mb-6 p-3 bg-[#111214] rounded-lg border border-[#1F2226]">
        ...
        <button
          onClick={fetchLeads}
          disabled={loading}
          className="flex items-center gap-2 px-3 py-1.5 bg-[#1F2226] hover:bg-[#2A2D31] text-[#9BA3AF] rounded text-sm transition-colors disabled:opacity-50"
        >
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center h-64 text-[#6B7280]">
          ...
          <p className="mb-2">No hot leads found</p>
          <p className="text-sm opacity-70">Hot leads are scored 80+ with status &quot;new&quot;</p>
        </div>
      ) : (
