      ...
      <div className="mb-6">
        ...
        <div className="flex flex-wrap items-center gap-4 mt-3 text-xs text-[#6B7280]">
          ...
          {task.parent_task_id && (
            <>
              <span>•</span>
              <Link href={`/tasks/${task.parent_task_id}`} className="flex items-center gap-1 text-[#9BA3AF] hover:text-white">
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    {children.map((child: any) => (
                      <Link key={child.id} href={`/tasks/${child.id}`} className="flex items-center justify-between p-2 bg-[#0B0B0C] rounded border border-[#1F2226] hover:border-[#3B82F6]/50">
                        <div className="flex items-center gap-2">
                ...
                {task.blocked_by?.length > 0 ? (
                  <div className="space-y-1">
                    {task.blocked_by.map((blocker: any) => (
                      <Link key={blocker.id} href={`/tasks/${blocker.id}`} className="text-xs text-[#FF3B30] hover:text-[#FF6A6A] block">{blocker.title}</Link>
                    ))}
                  </div>
                ...
                {task.waiting_on?.length > 0 ? (
                  <div className="space-y-1">
                    {task.waiting_on.map((wait: any) => (
                      <Link key={wait.id} href={`/tasks/${wait.id}`} className="text-xs text-[#FFB020] hover:text-[#FFCA4D] block">{wait.title}</Link>
                    ))}
                  </div>
